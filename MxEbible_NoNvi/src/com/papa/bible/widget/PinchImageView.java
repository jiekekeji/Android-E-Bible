...
public class PinchImageView extends ImageView {
	 ...
	 * 获取外部变换矩阵.
	 * 
	 * 外部变换矩阵记录了图片手势操作的最终结果,是相对于图片fit center状态的变换. 默认值为单位矩阵,此时图片为fit center状态.
	 * 
	 * @param matrix
	 *            用于填充结果的对象
	...
	public Matrix getInnerMatrix(Matrix matrix) {
		...
		if (isReady()) {
			// 原图大小
			RectF tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable()
					.getIntrinsicHeight());
			// 控件大小
	 ...
	 * 
	 * 调用此方法会停止正在进行中的手势以及手势动画. 当duration为0时,outerMatrix值会被立即设置而不会启动动画.
	 * 
	 * @param endMatrix
	 *            动画目标矩阵
	 * @param duration
	 ...
	 * 
	 * 调用此方法不会停止手势以及手势相关动画,但会停止正在进行的mask动画. 当前mask为null时,则不执行动画立即设置为目标mask.
	...
	public interface OuterMatrixChangedListener {
		 ...
		 * 外部矩阵变化回调
		 * 
		 * 外部矩阵的任何变化后都收到此回调. 外部矩阵变化后,总变化矩阵,图片的展示位置都将发生变化.
		 * 
	 ...
	 * 获取图片最大可放大的比例
	 * 
	 * 如果放大大于这个比例则不被允许. 在双手缩放过程中如果图片放大比例大于这个值,手指释放将回弹到这个比例.
	 * 在双击放大过程中不允许放大比例大于这个值. 覆盖此方法可以定制不同情况使用不同的最大可放大比例.
	 * 
	 * @return 缩放比例
	 * 
	 * @see #scaleEnd()
	 ...
	 * 
	 * 双指模式下: 记录两个手指的中点,作为和mScaleCenter绑定的点. 这个绑定可以保证mScaleCenter无论如何都会跟随这个中点.
	 * 
	 * @see #mScaleCenter
	 ...
	 * 缩放模式下图片的缩放中点.
	 * 
	 * 为其指代的点经过innerMatrix变换之后的值. 其指代的点在手势过程中始终跟随mLastMovePoint.
	 * 通过双指缩放时,其为缩放中心点.
	 * 
	 ...
	 * 双击后放大或者缩小
	 * 
	 * 将图片缩放比例缩放到nextScale指定的值. 但nextScale值不能大于最大缩放值不能小于fit center情况下的缩放值.
	 * 将双击的点尽量移动到控件中心.
	 * 
	 * @param x
	 *            双击的点
	...
	private void doubleTap(float x, float y) {
		...
		RectF testBound = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable()
				.getIntrinsicHeight());
	 ...
	 * 当缩放操作结束动画
	 * 
	 * 如果图片超过边界,找到最近的位置动画恢复. 如果图片缩放尺寸超过最大值或者最小值,找到最近的值动画恢复.
	 */
	 ...
	 * 对象池
	 * 
	 * 防止频繁new对象产生内存抖动. 由于对象池最大长度限制,如果吞度量超过对象池容量,仍然会发生抖动. 此时需要增大对象池容量,但是会占用更多内存.
	 * 
	 * @param <T>
	 *            对象池容纳的对象类型
	 */
	private static abstract class ObjectsPool<T> {
		 ...
		 * 获取一个空闲的对象
		 * 
		 * 如果对象池为空,则对象池自己会new一个返回. 如果对象池内有对象,则取一个已存在的返回.
		 * take出来的对象用完要记得调用given归还. 如果不归还,让然会发生内存抖动,但不会引起泄漏.
		 * 
		 * @return 可用的对象
		 * 
		 * @see #given(Object)
	...
	public static class MathUtils {
		 ...
		 * 计算两个矩形之间的变换矩阵
		 * 
		 * unknownMatrix.mapRect(to, from) 已知from矩形和to矩形,求unknownMatrix
		 * 
		 * @param from
		 * @param to
		 * @param result
		 ...
		 *            图片的高度
		 * @param scaleType
		 *            图片在ImageView中的ScaleType
		 * @param result
		 *            图片应该在ImageView中展示的矩形
		 */
		public static void calculateScaledRectInContainer(RectF container, float srcWidth, float srcHeight,
				ScaleType scaleType, RectF result) {
