...
public class WheelView<T> extends ListView implements IWheelView<T> {
		...
		public void handleMessage(Message msg) {
			if (msg.what == WheelConstants.WHEEL_SCROLL_HANDLER_WHAT) {
				...
				if (mJoinWheelView != null) {
					if (!WheelUtils.isEmpty(mJoinList) && mJoinList.size() == mList.size()) {
						mJoinWheelView.resetDataFromTop(mJoinList.get(getCurrentPosition()));
					} else {
	...
	private void setBackground() {
		Drawable drawable = DrawableFactory.createDrawable(mSkin, getWidth(), mItemH * mWheelSize, mStyle, mWheelSize,
				mItemH);
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
...
//		}
	...
	private void refreshCurrentPosition() {
		...
		mHandler.sendEmptyMessageDelayed(WheelConstants.WHEEL_SCROLL_HANDLER_WHAT,
				WheelConstants.WHEEL_SCROLL_DELAY_DURATION);
	}
	...
	private void refreshTextView(int position, int curPosition, View itemView, TextView textView) {
		if (curPosition == position) { // 选中
			int textColor = mStyle.selectedTextColor != -1 ? mStyle.selectedTextColor
					: (mStyle.textColor != -1 ? mStyle.textColor : WheelConstants.WHEEL_TEXT_COLOR);
			int textSize = mStyle.selectedTextSize != -1 ? mStyle.selectedTextSize
					: (mStyle.textSize != -1 ? mStyle.textSize : WheelConstants.WHEEL_TEXT_SIZE);
			...
			int textSize = mStyle.textSize != -1 ? mStyle.textSize : WheelConstants.WHEEL_TEXT_SIZE;
			int delta = Math.abs(position - curPosition);
			float alpha = (float) Math.pow(mStyle.textAlpha != -1 ? mStyle.textAlpha : WheelConstants.WHEEL_TEXT_ALPHA,
					delta);
